...
// Modifications Copyright 2018 The OPA Authors. All rights reserved.
...
// GetEntries returns the stringmap's entries field
func (s StringMap) GetEntries() map[string]string {
	return s.entries
}
...
func (pb *ProtoBag) Get(name string) (interface{}, bool) {
	...
	if !ok {
		fmt.Printf("Attribute '%s' not in either global or message dictionaries", name)
...
func (pb *ProtoBag) internalGet(name string, index int32) (interface{}, bool) {
	...
	if ok {
		...
		if err != nil {
			fmt.Printf("string attribute %s: %v", name, err)
			return nil, false
		}
	...
	if ok {
		...
		if err != nil {
			fmt.Printf("string map %s: %v", name, err)
			return nil, false
		}
